...
pub fn init_ltdc() {
    ...
    // Configure sync and porch timings
    ltdc.sscr.write(|w| w
        .hsw().bits((HSYNC - 1) as u16)
        .vsh().bits((VSYNC - 1) as u16)
    );
    ltdc.bpcr.write(|w| w
        .ahbp().bits((HSYNC + HBP - 1) as u16)
        .avbp().bits((VSYNC + VBP - 1) as u16)
    );
    ltdc.awcr.write(|w| w
        .aaw().bits((HSYNC + HBP + LCD_WIDTH - 1) as u16)
        .aah().bits((VSYNC + VBP + LCD_HEIGHT - 1) as u16)
    );
    ltdc.twcr.write(|w| w
        .totalw().bits((HSYNC + HBP + LCD_WIDTH + HFP - 1) as u16)
        .totalh().bits((VSYNC + VBP + LCD_HEIGHT + VFP - 1) as u16)
    );
    ...
    ltdc.layer1.cfbar.write(|w| w.cfbadd().bits(LAYER1_BASE));
        ...
        ltdc.layer1.cfblr.write(|w| w
            .cfbp().bits(pitch)
            .cfbll().bits(line_len)
        );
        // Number of lines
    ltdc.layer1.cfblnr.write(|w| w.cfblnbr().bits(LCD_HEIGHT as u16));
        // Alpha and blending
    ltdc.layer1.cacr.write(|w| w.consta().bits(0xFF));
        ltdc.layer1.bfcr.write(|w| unsafe { w
            .bf1().bits(6) // BF1: pixel alpha x const alpha
            .bf2().bits(7) // BF2: pixel alpha x const alpha
        });
        // Enable layer
    ...
    // Layer 2 config (ARGB8888, 64x64)
   {
    ...
    ltdc.layer2.cfbar.write(|w| w.cfbadd().bits(LAYER2_BASE));
        ...
        let pitch = pitch_bytes as u16;
        let line_len = (pitch_bytes + 3) as u16;
        ltdc.layer2.cfblr.write(|w| w
            .cfbp().bits(pitch)
            .cfbll().bits(line_len)
        );
        // Number of lines
    ltdc.layer2.cfblnr.write(|w| w.cfblnbr().bits(LAYER2_H as u16));
        // Alpha and blending
        #[cfg(feature = "l2-opaque")]
        {
            // Use constant alpha only: BF1=CA, BF2=1-CA
            ltdc.layer2.cacr.write(|w| w.consta().bits(0xFF));
            ltdc.layer2.bfcr.write(|w| unsafe { w
                .bf1().bits(4) // CA
                .bf2().bits(5) // 1-CA
            });
        }
...
pub fn set_layer2_position(x: u32, y: u32) {
    use core::cmp::min;
    let dp = unsafe { pac::Peripherals::steal() };
    let ltdc = dp.LTDC;
...
pub fn layer1_back_addr() -> u32 { unsafe { if L1_FRONT == LAYER1_BASE { LAYER1_BASE_B } else { LAYER1_BASE } } }

// Swap Layer1 front/back by updating CFBAR to the back buffer and latching on VBlank
#[cfg(feature = "overlay")]
pub fn swap_layer1_buffers() {
    let dp = unsafe { pac::Peripherals::steal() };
    let ltdc = dp.LTDC;
    let new_front = layer1_back_addr();
    ltdc.layer1.cfbar.write(|w| w.cfbadd().bits(new_front));
    // Latch address change at next VBlank
    ltdc.srcr.modify(|_, w| w.vbr().set_bit());
    // Update tracker
    unsafe { L1_FRONT = new_front; }
}
...
fn setup_ltdc_gpio() {
    ...
    fn set_af(gpio_base: *const pac::gpioa::RegisterBlock, pin: u32, af: u32) {
        ...
        gpio.moder.modify(|r, w| unsafe { w.bits((r.bits() & !(0b11 << (pin*2))) | (0b10 << (pin*2))) });
        // Fast (50MHz) speed to match C example
        gpio.ospeedr.modify(|r, w| unsafe { w.bits((r.bits() & !(0b11 << (pin*2))) | (0b10 << (pin*2))) });
        // Push-pull
        gpio.otyper.modify(|r, w| unsafe { w.bits(r.bits() & !(1 << pin)) });
        // No pull-up/down
        gpio.pupdr.modify(|r, w| unsafe { w.bits(r.bits() & !(0b11 << (pin*2))) });
        // AF low/high
        if pin < 8 {
            let idx = pin;
            gpio.afrl.modify(|r, w| unsafe {
                let mut v = r.bits(); v &= !(0xF << (idx*4)); v |= af << (idx*4); w.bits(v)
            });
        } else {
            let idx = pin - 8;
    ...
    set_af(pb, 1, 9);
    set_af(pg, 6, 14);
    // G2=PA6(AF14), G3=PG10(AF9), G4=PB10(AF14), G5=PB11(AF14), G6=PC7(AF14), G7=PD3(AF14)
    set_af(pa, 6, 14);
    set_af(pg, 10, 9);
    set_af(pb, 10, 14);
    set_af(pb, 11, 14);
