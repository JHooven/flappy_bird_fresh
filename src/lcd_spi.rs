...
fn spi5() -> pac::SPI5 {
    unsafe { pac::Peripherals::steal().SPI5 }
}
...
fn lcd_command(cmd: u8, delay_ms: u16, data: &[u8]) {
    ...
    // crude delay
    if delay_ms != 0 {
        crate::clock::delay_ms(delay_ms as u32);
    }
}
...
pub fn init() {
    ...
    gpioc
        .moder
        .modify(|r, w| unsafe { w.bits((r.bits() & !(0b11 << (2 * 2))) | (0b01 << (2 * 2))) });
    gpioc
        .ospeedr
        .modify(|r, w| unsafe { w.bits((r.bits() & !(0b11 << (2 * 2))) | (0b10 << (2 * 2))) });
    // PD13 output
    gpiod
        .moder
        .modify(|r, w| unsafe { w.bits((r.bits() & !(0b11 << (13 * 2))) | (0b01 << (13 * 2))) });
    gpiod
        .ospeedr
        .modify(|r, w| unsafe { w.bits((r.bits() & !(0b11 << (13 * 2))) | (0b10 << (13 * 2))) });
    // PF7, PF9 AF5
    for &pin in &[7u32, 9u32] {
        gpiof.moder.modify(|r, w| unsafe {
            w.bits((r.bits() & !(0b11 << (pin * 2))) | (0b10 << (pin * 2)))
        });
        gpiof.ospeedr.modify(|r, w| unsafe {
            w.bits((r.bits() & !(0b11 << (pin * 2))) | (0b10 << (pin * 2)))
        });
        if pin < 8 {
            let idx = pin;
    ...
    // Keep portrait orientation for LTDC: MADCTL = BGR (0x08)
    // Since we're using LTDC framebuffer directly, handle rotation in software only
